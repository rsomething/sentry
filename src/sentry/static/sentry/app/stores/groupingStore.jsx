...
const GroupingStore = Reflux.createStore({
  ...
  onToggleUnmerge([fingerprint, eventId]) {
    ...
    let state = this.unmergeState.has(fingerprint) && this.unmergeState.get(fingerprint);
    ...
    if (this.unmergeList.has(fingerprint)) {
      ...
      this.unmergeList.set(fingerprint, [fingerprint, eventId]);
      checked = true;
    }
  ...
  onUnmerge({groupId, loadingMessage, successMessage, errorMessage}) {
    let ids = Array.from(this.unmergeList.values()).map(([fingerprint]) => fingerprint);
